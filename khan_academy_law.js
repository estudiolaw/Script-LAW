(async () => {
    ...
    overlay.innerHTML = `
      ...
      ">
        ...
        <button id="law-password-btn" style="
          ...
          margin-bottom:10px;
        ">Entrar</button>
        <p style="margin:5px 0 20px;color:#ccc;font-size:.9em;">Feito por: Estúdio LAW</p>
  ...
  function addFireworks() {
    ...
    const interval = setInterval(() => {
      ...
      Object.assign(rocket.style, {
        ...
        width:'4px',height:'12px',background:'white',
        borderRadius:'2px',
        animation:`rocket ${1.5 + Math.random()}s ease-out forwards`
      });
      ...
      rocket.addEventListener('animationend', () => {
        ...
        for (let i=0; i<count; i++) {
          ...
          Object.assign(spark.style, {
            ...
            top:(window.innerHeight - rocket.offsetTop)+'px',
            width:'3px',height:'3px',background:`hsl(${Math.random()*360},100%,75%)`,
            borderRadius:'50%',
          ...
          const move = setInterval(() => {
            t += 16;
            spark.style.transform = `translate(${vx* t/100}px, ${-vy* t/100}px)`;
            if (t>1000) clearInterval(move);
          },16);
  ...
  btn.addEventListener('click', async () => {
    ...
    if (res===true) {
      ...
      clearInterval(fw.dataset.interval);
      fw.remove();
      showEstudioLaw();
    } else if (res==='expired') {
  ...
  // --- Se não for Khan Academy, redireciona ---
