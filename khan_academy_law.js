(async () => {
  ...
  // --- Cria tela de fundo animado após a comemoração ---
  function createAnimatedBackground() {
    ...
    bg.style.background = 'linear-gradient(45deg, #ff2a68, #00bcd4)';
    bg.style.animation = 'gradientAnimation 10s ease infinite';
    document.body.appendChild(bg);
    
    const style = document.createElement('style');
    style.textContent = `
      @keyframes gradientAnimation {
        0% { background: linear-gradient(45deg, #ff2a68, #00bcd4); }
        50% { background: linear-gradient(45deg, #8a2be2, #ff2a68); }
        100% { background: linear-gradient(45deg, #00bcd4, #8a2be2); }
      }
    `;
